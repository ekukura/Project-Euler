...
#start_p should be the smaller part a in the factorization a*b = num.
#(e.g. start_p should be <= floor(sqrt(num))
...
#assume num <= den
#num and dem are the numerator and denominator, resp., of the number
...
def recip_cycle(n): 
    ...
    while not is_cycle and not cur_numer == 0:
        ...
        if cur_numer == 0: #not a cycle, so erase whatever has been accumulated
            decimal_cycle = list() #cur_numer = 0 will then stop the loop as well
...
if __name__ == '__main__':   
    ...
    print("The maximum length numerator cycle is: {}\n\n"
